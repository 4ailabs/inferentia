...
import PacienteProgramaClient from "./paciente-programa-client";
...
export async function generateMetadata({
  ...
  return {
    title:
      locale === "es"
        ? "Tu programa · Paciente — Inferentia"
        : "Your program · Patient — Inferentia",
  };
}

export default async function PacienteProgramaPage({
  searchParams,
}: {
  searchParams: SearchParams;
}) {
  ...
  return (
    <main className="min-h-screen bg-paper text-ink">
        ...
        <div className="mx-auto max-w-[1280px] px-6 md:px-10">
          <div className="flex items-end justify-between py-5">
            <div className="flex items-end gap-6">
              ...
              <p className="eyebrow">
                {locale === "es" ? "Paciente · Tu programa" : "Patient · Your program"}
              </p>
            </div>
            <LocaleToggle locale={locale} pathname="/paciente/programa" search={sp} />
          </div>
        </div>
      ...
      <article className="mx-auto max-w-[1000px] px-6 md:px-10 py-10 pb-16">
        <PacienteProgramaClient initialLocale={locale} />
      </article>
